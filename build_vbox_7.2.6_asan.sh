...
usage() {
    cat <<EOF
Usage: $(basename "$0") [options] [-- <kmk args...>]

Download VirtualBox ${VERSION}, apply the local clang patch, and build the
ASAN-instrumented userspace targets via vbox_with_clang/build-asan.sh.
...
Environment:
  JOBS=<n>                          Passed through to vbox_with_clang/build-asan.sh.
  OUT_BASE_DIR=<dir>                Override the ASAN build output directory.
  VBOX_WITH_GCC_SANITIZER_STATIC=1  Link the sanitizer runtime statically.

Examples:
  $(basename "$0")
  JOBS=32 $(basename "$0")
  OUT_BASE_DIR=/tmp/vbox-asan $(basename "$0")
  VBOX_WITH_GCC_SANITIZER_STATIC=1 $(basename "$0")
EOF
}
...
[[ -f "$HELPER_DIR/build-asan.sh" ]] || die "Missing helper script: ${HELPER_DIR}/build-asan.sh"
...
log "Starting ASAN build in ${SOURCE_DIR}"
bash "$HELPER_DIR/build-asan.sh" "$SOURCE_DIR" "$@"
